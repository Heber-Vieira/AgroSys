...
export const DraggableHelpButton: React.FC<DraggableHelpButtonProps> = ({ 
  ...
  const getBottomRightPosition = useCallback((): Position => {
    const btnWidth = buttonRef.current?.offsetWidth || 90;
    const btnHeight = buttonRef.current?.offsetHeight || 36;
    const margin = 12;
    return {
  ...
  // Reset position to default when view changes (ignores old local storage)
  useEffect(() => {
    setPosition(getBottomRightPosition());
  }, [currentView, getBottomRightPosition]);
  ...
  return (
    <button
      ...
      className={`fixed z-50 px-3 py-1.5 rounded-full font-black text-[11px] text-white shadow-2xl border transition-[transform,shadow,background-color] duration-75 flex items-center gap-1.5 select-none ${
        isDragging
      ...
      title="Ajuda (Clique para abrir • Segure e arraste com o mouse para reposicionar)"
      aria-label="Ajuda - Botão flutuante arrastável"
    >
      <GripVertical className="w-3 h-3 text-emerald-200/80 -ml-0.5 pointer-events-none" />
      <HelpCircle className="w-3.5 h-3.5 text-white pointer-events-none" />
