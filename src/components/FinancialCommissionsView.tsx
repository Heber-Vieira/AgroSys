...
export const FinancialCommissionsView: React.FC<FinancialCommissionsViewProps> = ({
  ...
  const displayedEntries = financials.filter(f => {
    if (currentUser.role === 'PILOT') {
      return f.category === 'COMISSAO_PILOTO' || f.clientOrBeneficiary.includes('Diego') || f.clientOrBeneficiary.includes('Piloto');
    }
    if (currentUser.role === 'ASSISTANT') {
      return f.category === 'COMISSAO_AUXILIAR' || f.clientOrBeneficiary.includes('Lucas') || f.clientOrBeneficiary.includes('Auxiliar');
    }
    if (currentUser.role === 'USER') {
      return f.type === 'RECEIVABLE' && (f.clientOrBeneficiary.includes(currentUser.name) || f.clientOrBeneficiary.includes('Santa Fé'));
    }
