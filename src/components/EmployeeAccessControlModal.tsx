...
export const EmployeeAccessControlModal: React.FC<EmployeeAccessControlModalProps> = ({
  ...
  const [selectedViews, setSelectedViews] = useState<AppViewMode[]>(() => {
    ...
    if (selectedUser.allowedViews && Array.isArray(selectedUser.allowedViews)) {
      return [...selectedUser.allowedViews];
    }
    return getDefaultRoleViews(selectedUser.role);
  });

  // When selected user changes, re-populate views
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-sm flex items-center justify-center p-3 sm:p-6 overflow-y-auto animate-in fade-in duration-150">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-4xl w-full p-5 sm:p-6 shadow-2xl space-y-5 text-white my-auto max-h-[92vh] overflow-y-auto">
        {/* Header */}
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 max-h-48 overflow-y-auto p-1 bg-slate-950/60 rounded-2xl border border-slate-800">
            {manageableUsers.map((u) => {
              const isSelected = selectedUser?.id === u.id;
              const hasCustom = u.allowedViews && u.allowedViews.length > 0;
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-[10px] text-slate-400 font-mono truncate">{u.roleLabel}</div>
                  </div>
        ...
        {selectedUser && (
          <>
            ...
            <div className="bg-slate-950/80 p-3.5 rounded-2xl border border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
              <div className="flex items-center gap-3">
                <UserAvatar user={selectedUser} size="md" />
                <div>
                  <span className="font-bold text-white text-sm block">{selectedUser.name}</span>
                  <span className="text-slate-400 text-xs">{selectedUser.email} • Cargo: <strong className="text-emerald-400">{selectedUser.roleLabel}</strong></span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl text-xs font-semibold bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 flex items-center gap-1.5 cursor-pointer"
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl text-xs font-semibold bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-300 border border-emerald-500/30 flex items-center gap-1.5 cursor-pointer"
                >
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-1 mb-0.5">
                          <h4 className={`text-xs font-black truncate ${isChecked ? 'text-white' : 'text-slate-400'}`}>
                            {mod.name}
                          </h4>
                        </div>
